...
const CommandesEnAttente = () => {
    ...
    const [url, setUrl] = useState(`${process.env.REACT_APP_NODE_URL}/api/v1/admin/commande?estApprouve=false`)
    useEffect(() => {
        fetch(url, {
            method: "GET",
            headers: {
                ...
                <tbody style={{ fontSize: '14px' }}>
                    {commandes && commandes.map((commande, key) => (
                        <tr key={key}>
                            ...
                            <td>
                                {commande.informationCommande.nomEntreprise}
                            </td>
                            <td style={{ textOverflow: 'ellipsis', maxWidth: '15ch' }}>
                                {commande.informationCommande.description}
                            </td>
                            <td>
                                {commande.informationPaiement.nomTarif}
                            </td>
                            <td>
                                <span onClick={()=>{handleValider(commande._id)}} style={{fontSize: '12px'}} className="btn btn-success">Valider</span>
                            </td>
                        </tr>
                    ))}
                </tbody>
